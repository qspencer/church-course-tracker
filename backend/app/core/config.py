...
class Settings(BaseSettings):
    ...
    # Planning Center API
    PLANNING_CENTER_API_URL: str = os.getenv(
    ...
    USE_MOCK_PLANNING_CENTER: bool = (
        os.getenv("USE_MOCK_PLANNING_CENTER", "true").lower() == "true"
    )
    ...
    def _validate_security_settings(self):
        ...
        if self.ENVIRONMENT == "production":
            ...
            if "*" in self.ALLOWED_HOSTS:
                logger.warning(
                    "Wildcard (*) in ALLOWED_HOSTS is not recommended for production"
                )
        ...
        if self.ACCESS_TOKEN_EXPIRE_MINUTES > 1440:  # 24 hours
            logger.warning(
                "ACCESS_TOKEN_EXPIRE_MINUTES is very long - consider shorter expiration"
            )
