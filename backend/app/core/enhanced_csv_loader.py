...
class EnhancedCSVDataLoader:
    ...
    def get_csv_data_summary(self, db: Session) -> Dict[str, int]:
        ...
        """
        return {
            "csv_campuses": db.query(Campus)
            .filter(Campus.data_source == "csv")
            .count(),
            ...
            "csv_modules": db.query(CourseModule)
            .filter(CourseModule.data_source == "csv")
            .count(),
            "csv_content": db.query(CourseContent)
            .filter(CourseContent.data_source == "csv")
            .count(),
            "csv_enrollments": db.query(CourseEnrollment)
            .filter(CourseEnrollment.data_source == "csv")
            .count(),
        }
    ...
    def _load_campuses(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(Campus).filter(Campus.data_source == "csv").first()
        ):
    ...
    def _load_roles(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(Role).filter(Role.data_source == "csv").first()
        ):
    ...
    def _load_users(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(User).filter(User.data_source == "csv").first()
        ):
    ...
    def _load_people(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(People).filter(People.data_source == "csv").first()
        ):
    ...
    def _load_courses(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(Course).filter(Course.data_source == "csv").first()
        ):
    ...
    def _load_modules(self, db: Session, force_reload: bool) -> int:
        ...
        if (
            not force_reload
            and db.query(CourseModule).filter(CourseModule.data_source == "csv").first()
        ):
    ...
    def _load_enrollments(self, db: Session, force_reload: bool) -> int:
        ...
        with open(csv_file, "r", encoding="utf-8") as file:
            ...
            for row in reader:
                ...
                if not person:
                    logger.warning(
                        f"Person not found: {row['first_name']} {row['last_name']}"
                    )
                    continue
    ...
    def _clear_csv_enrollments(self, db: Session) -> int:
        ...
        db.query(CourseEnrollment).filter(
            CourseEnrollment.data_source == "csv"
        ).delete()
    ...
    def _clear_csv_content(self, db: Session) -> int:
        ...
        count = (
            db.query(CourseContent).filter(CourseContent.data_source == "csv").count()
        )
