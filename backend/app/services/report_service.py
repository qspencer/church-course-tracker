...
from app.schemas.report import ReportData, ReportResponse, ReportType
...
class ReportService:
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        active_courses = (
            self.db.query(func.count(CourseModel.id))
            .filter(CourseModel.is_active == True)
            .scalar()
            or 0
        )
        ...
        completed_enrollments = (
            self.db.query(func.count(EnrollmentModel.id))
            .filter(EnrollmentModel.status == "completed")
            .scalar()
            or 0
        )
        ...
        # Get total members
        total_members = (
            self.db.query(func.count(PeopleModel.id))
            .filter(PeopleModel.is_active == True)
            .scalar()
            or 0
        )
    ...
    def generate_member_progress_report(
        self, member_id: Optional[int] = None, course_id: Optional[int] = None
    ) -> ReportResponse:
    ...
    def _count_by_status(self, enrollments: List[EnrollmentModel]) -> Dict[str, int]:
        ...
        for enrollment in enrollments:
            status_counts[enrollment.status] = (
                status_counts.get(enrollment.status, 0) + 1
            )
