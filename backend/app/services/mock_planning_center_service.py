...
class MockPlanningCenterService:
    ...
    def _generate_mock_data(self) -> Dict[str, Any]:
        ...
        for i in range(50):  # Generate 50 mock people
            person = {
                ...
                "date_of_birth": (
                    date.today() - timedelta(days=random.randint(18 * 365, 80 * 365))
                ).isoformat(),
                "gender": random.choice(["Male", "Female", "Other"]),
                "address1": f"{random.randint(100, 9999)} {random.choice(['Main', 'Oak', 'Pine', 'Maple', 'Cedar'])} St",
                "city": random.choice(
                    ["Anytown", "Springfield", "Riverside", "Hillside", "Valley"]
                ),
                ...
                "join_date": (
                    date.today() - timedelta(days=random.randint(30, 3650))
                ).isoformat(),
                "created_at": (
    ...
    async def get_person(self, person_id: str) -> Dict[str, Any]:
        ...
        person = next(
            (p for p in self.mock_data["people"] if p["id"] == person_id), None
        )
        if not person:
            raise HTTPException(
    ...
    async def get_event_registrations(
        ...
        registrations = [
            r for r in self.mock_data["registrations"] if r["event_id"] == event_id
        ]
    ...
    async def get_person_registrations(
        self, person_id: str, limit: int = 50, offset: int = 0
    ) -> Dict[str, Any]:
        """Get registrations for a specific person"""
        registrations = [
            r for r in self.mock_data["registrations"] if r["person_id"] == person_id
        ]
        paginated_registrations = registrations[offset : offset + limit]
    ...
    async def create_registration(
        self, event_id: str, person_id: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        existing_registration = next(
            (
                r
                for r in self.mock_data["registrations"]
                if r["person_id"] == person_id and r["event_id"] == event_id
            ),
            None,
        )
    ...
    async def update_registration(
        ...
        registration = next(
            (r for r in self.mock_data["registrations"] if r["id"] == registration_id),
            None,
        )
    ...
    def get_mock_data_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_people": len(
                [p for p in self.mock_data["people"] if p["status"] == "active"]
            ),
            "upcoming_events": len(
                [
                    e
                    for e in self.mock_data["events"]
                    if datetime.fromisoformat(e["start_time"]) > datetime.now()
                ]
            ),
            "recent_registrations": len(
                [
                    r
                    for r in self.mock_data["registrations"]
                    if datetime.fromisoformat(r["registration_date"])
