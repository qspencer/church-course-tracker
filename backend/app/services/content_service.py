...
class ContentService:
    ...
    def _init_s3_client(self):
        ...
        if (
            ...
            and hasattr(settings, "AWS_S3_BUCKET")
        ):
            try:
    ...
    def create_module(
        self, module_data: CourseModuleCreate, user_id: int
    ) -> CourseModule:
        """Create a new course module"""
        # Verify course exists
        course = (
            self.db.query(Course).filter(Course.id == module_data.course_id).first()
        )
        if not course:
            raise HTTPException(
    ...
    def update_module(
        self, module_id: int, module_data: CourseModuleUpdate, user_id: int
    ...
    def create_content(
        self, content_data: CourseContentCreate, user_id: int
    ) -> CourseContent:
        """Create new course content"""
        # Verify course exists
        course = (
            self.db.query(Course).filter(Course.id == content_data.course_id).first()
        )
        if not course:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Course not found"
    ...
    def upload_file(
        self, content_id: int, file: UploadFile, user_id: int
    ) -> Dict[str, Any]:
        """Upload a file for course content"""
        content = (
            self.db.query(CourseContent).filter(CourseContent.id == content_id).first()
        )
        if not content:
            raise HTTPException(
    ...
    def get_content(
        self, course_id: int, module_id: Optional[int] = None
    ) -> List[CourseContent]:
    ...
    def update_content(
        self, content_id: int, content_data: CourseContentUpdate, user_id: int
    ...
    def log_content_access(
        self, access_data: ContentAccessLogCreate
    ) -> ContentAccessLog:
        """Log content access"""
    ...
    def get_content_access_logs(
        self, content_id: int, limit: int = 100
    ) -> List[ContentAccessLog]:
        """Get access logs for content"""
        return (
            self.db.query(ContentAccessLog)
            .filter(ContentAccessLog.content_id == content_id)
            .order_by(desc(ContentAccessLog.access_timestamp))
            .limit(limit)
            .all()
        )
    ...
    def get_audit_logs(
        ...
        return (
            self.db.query(ContentAuditLog)
            .filter(ContentAuditLog.content_id == content_id)
            .order_by(desc(ContentAuditLog.change_timestamp))
            .limit(limit)
            .all()
        )
    ...
    def _determine_storage_type(self, content_data: CourseContentCreate) -> StorageType:
        """Determine storage type based on content type"""
        if content_data.content_type in [
            ContentType.EXTERNAL_LINK,
            ContentType.EMBEDDED,
        ]:
            return StorageType.EXTERNAL
        elif (
            content_data.file_size and content_data.file_size > 10 * 1024 * 1024
        ):  # 10MB
    ...
    def _store_file_in_s3(
        self, filename: str, file_content: bytes, content_type: str
    ) -> str:
        """Store file in S3"""
        ...
        if storage_type == StorageType.DATABASE:
            ...
            try:
                self.s3_client.delete_object(
                    Bucket=settings.AWS_S3_BUCKET, Key=file_path
                )
            except ClientError:
