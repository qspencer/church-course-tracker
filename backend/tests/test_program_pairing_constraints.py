    ...
    def test_create_pairing_with_max_secondary_limit(self, client, db_session, admin_token):
        ...
        try:
            ...
            if 'program' in locals():
                db_session.query(ProgramPairing).filter(ProgramPairing.program_id == program.id).delete()
                db_session.query(ProgramParticipant).filter(ProgramParticipant.program_id == program.id).delete()
                db_session.query(People).filter(People.planning_center_id.in_(["pc_test_1", "pc_test_2", "pc_test_3", "pc_test_4"])).delete()
    ...
    def test_create_pairing_with_multiple_secondary_not_allowed(self, client, db_session, admin_token):
        ...
        try:
            ...
            if 'program' in locals():
                db_session.query(ProgramPairing).filter(ProgramPairing.program_id == program.id).delete()
                db_session.query(ProgramParticipant).filter(ProgramParticipant.program_id == program.id).delete()
                db_session.query(People).filter(People.planning_center_id.in_(["pc_test_1", "pc_test_2", "pc_test_3"])).delete()
                db_session.delete(program)
                db_session.commit()
